...
def pickThree(beerRDD):
	...
	@rtype:	list
	"""
	topThreeBeer = beerRDD.map(lambda x: (x.beer_name, (x.review_overall, 1))).reduceByKey(lambda x,y: (x[0] + y[0], x[1] + y[1])).filter(lambda x: x[1][1] > 1).mapValues(lambda x: ((x[0] + 0.0) / x[1])).top(3, key = lambda x: x[1])
	return topThreeBeer
